...
function Database() {
    ...
    this.getStudentsFromCourse = function (courseId, callback) {
        Course.findOne({_id: courseId}).populate('students', 'name').exec(function(error, course) {
            if (error)
    ...
    this.getRosterInputStatus = function (courseId, callback) {
        Course.findOne({_id: courseId}).populate('tests').populate('students').exec(function(error, course) {
            var rosterStatus = [];
            for (var i = 0; i < course.students.length; i++) {
                var studentObject = new Object();
                var studentId = course.students[i]._id;
                studentObject.id = studentId;
                var studentInputStatus = {};
                for (var j = 0; j < course.tests.length; j++) {
                    var testId = course.tests[j]._id;
                    Report.findOne({student_id: studentId, test_id: testId}, function(error, report) {
                        if (report != null)
                            studentInputStatus[testId] = true;
                        else
                            studentInputStatus[testId] = false;
                    });
                }
                studentObject.inputStatus = studentInputStatus;
                rosterStatus.push(studentObject);
            }
            callback(rosterStatus);
        });
    }
}
